...
export default class ShoppingCartList extends Component {
    ...
    if (ingredientsModification) {
      ...
      Object.keys(ingredientsModification).map((id) => {
        ...
        if (!isNullOrUndefined(modificationData.tags))
          result += modificationData.tags.map((tag) => {
            return `${tag.label}`;
          }).join(', ');
  ...
  render() {
    ...
    return (
      <ShoppingList>
        {store.shoppingCartStore.products.length > 0 && store.shoppingCartStore.products.map((item) => {
          ...
          return (
            <Product key={product.id}>
              ...
              <ListItemText
                ...
                secondary={this.extraOptionsLine(product, modificationData, ingredientsModification)}
              />
              <ListItemSecondaryAction>
                <IconButton aria-label="Delete" onClick={(e) => { return this.handleRemoveProduct(e, product) }}>
                  <DeleteIcon />
                </IconButton>
              </ListItemSecondaryAction>
            </Product>
